...
function placepress_filter_title($title,$id){
	if(is_singular('locations') && in_the_loop() && $id == get_the_ID()){
		$post = $GLOBALS['post'];
...
function placepress_filter_content($content){
	if(is_singular('locations') && in_the_loop() && is_main_query()){
		$post = $GLOBALS['post'];
		...
		$map = placepress_setting('content_map') && $includeMap ? placepress_location_map($post) : null;
		...
		$locations = placepress_locations_for_tour($post);
...
function placepress_disable_gutenberg($can_edit, $post_type) {
	...
	if ($post_type === 'locations') return false; // disable for locations
